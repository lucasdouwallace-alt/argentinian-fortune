...
export const Route = createFileRoute("/auth")({
  head: () => ({
    meta: [
      { title: "Ingresar · Oráculo" },
      { name: "description", content: "Ingresá a tu monitor de inversiones 24/7." },
    ],
  }),
  component: AuthPage,
});
    ...
    <div className="min-h-screen bg-glow flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        <Link to="/" className="flex items-center justify-center gap-2 mb-8">
          ...
          <span className="text-2xl font-display font-bold tracking-tight">Oráculo</span>
        </Link>
        ...
        <div className="bg-card border rounded-xl p-6 shadow-card">
          <div className="flex gap-2 mb-6 p-1 bg-secondary rounded-lg">
            <button
              ...
              className={`flex-1 py-2 rounded-md text-sm font-medium transition ${mode === "signin" ? "bg-card shadow-sm" : "text-muted-foreground"}`}
            >Ingresar</button>
            <button
              onClick={() => setMode("signup")}
              className={`flex-1 py-2 rounded-md text-sm font-medium transition ${mode === "signup" ? "bg-card shadow-sm" : "text-muted-foreground"}`}
            >Crear cuenta</button>
          </div>
